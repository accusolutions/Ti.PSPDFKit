...
- (void)setKeychainAsSignatureStore {
    [self.controller updateConfigurationWithBuilder:^(PSPDFConfigurationBuilder *builder) {
        builder.signatureSavingStrategy = PSPDFSignatureSavingStrategySaveIfSelected;
        builder.signatureStore = [[PSPDFKeychainSignatureStore alloc] init];
    }];
}

- (void)setSignatureCreationConfiguration:(id) args {
    NSDictionary *signatureCreationConfiguration = args;
    [self.controller updateConfigurationWithoutReloadingWithBuilder:^(PSPDFConfigurationBuilder *builder) {    
        /**
                 Since that signatureCreationConfiguration is a hasable, (Dictionary) we can simply iterate over the sent hash and set it accordingly.
         */
        [signatureCreationConfiguration enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
            [builder.signatureCreationConfiguration setValue:value forKey:key];
        }];
